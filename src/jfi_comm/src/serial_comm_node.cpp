...
: Node("serial_comm_node")
{
  ...
  sub_to_serial_poly_traj_ = this->create_subscription<jfi_comm::msg::PolyTraj>("/planning/broadcast_traj_send", qos,
      [this](const jfi_comm::msg::PolyTraj::SharedPtr msg)
      {
  ...
  pub_from_serial_poly_traj_ = this->create_publisher<jfi_comm::msg::PolyTraj>("/planning/broadcast_traj_recv", qos);
}
...
void SerialCommNode::handleMessage(const int tid, const std::vector<uint8_t> & data)
{
  switch (tid) {
    ...
    {
      try {
        jfi_comm::msg::PolyTraj polytraj_msg = jfi_comm_.deserialize_message<jfi_comm::msg::PolyTraj>(data);
