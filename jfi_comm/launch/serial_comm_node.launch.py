...
def generate_launch_description():
    ...
    port_arg = DeclareLaunchArgument(
        "port_name", default_value="/dev/ttyUSB0", description="Serial port device path"
    )
    baud_arg = DeclareLaunchArgument(
        "baud_rate", default_value="115200", description="Serial port baud rate"
    )
    system_id_arg = DeclareLaunchArgument(
        "system_id", default_value="1", description="MAVLink system ID"
    )
